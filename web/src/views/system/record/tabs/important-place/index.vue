<template>
  <div class="wrapper">
    ...
    <div class="plan-record-dialog-wrapper">
      <important-place-dialog
        :info="importantPlaceInfoActive"
        :visible.sync="importantPlaceDialogVisible"
        @update-list="getTableList"
        v-if="importantPlaceDialogVisible"
      ></important-place-dialog>
    </div>
  </div>
</template>
...
import importantPlaceDialog from './components/importantPlace/index'

export default {
  components: {
    importantPlaceDialog
  },
  data () {
    return {
      isLoading: false,
      importantPlaceDialogVisible: false,
      filterMenu: filterMenu,
      importantPlaceInfoActive: {},
      addBtnConfig: [],
      tableProps: [
        {
          label: '单位名称',
  ...
  computed: {
    ...mapState('sinuo/fireControl', {
      tableList: state => {
        return state.importantPlaceList
      }
    })
  },
  methods: {
    ...
    getTableList () {
      ...
      this.$store.dispatch('sinuo/fireControl/getImportantPlaceList', params).then(res => {
    ...
    editInfo (info = {}) {
      this.importantPlaceInfoActive = info
      this.importantPlaceDialogVisible = true
    },
    deleteInfo (info) {
      this.$confirm('此操作将对其进行永远删除，是否继续？', '提示', {
        ...
        this.$store
          .dispatch('sinuo/fireControl/deleteImportantPlace', params)
          .then(res => {
            this.getTableList()
          })
      })
    ...
    setCellStyle ({ row, column, rowIndex, columnIndex }) {
      let style = ''
      const detailIndex = this.tableProps.findIndex(item => {
