<template>
  <div class="wrapper">
    ...
    <div class="plan-record-dialog-wrapper">
      <fire-station-dialog
        :stationInfo="fireStationInfoActive"
        :visible.sync="fireStationDialogVisible"
        @update-list="getTableList"
        v-if="fireStationDialogVisible"
      ></fire-station-dialog>
    </div>
  </div>
</template>
...
import fireStationDialog from './components/fireStationDialog/index'
...
export default {
  ...
  data () {
    return {
      isLoading: false,
      fireStationDialogVisible: false,
      filterMenu: filterMenu,
      fireStationInfoActive: {},
      addBtnConfig: [],
      tableProps: [
        {
          label: '单位名称',
  ...
  methods: {
    ...
    getTableList () {
      ...
      this.$store.dispatch('sinuo/fireControl/getFireStationList', params).then(res => {
    ...
    editStation (info = {}) {
      this.fireStationInfoActive = info
      this.fireStationDialogVisible = true
    },
    deleteStation (info) {
      this.$confirm('此操作将对其进行永远删除，是否继续？', '提示', {
        ...
        this.$store
          .dispatch('sinuo/fireControl/deleteFireStation', params)
          .then(res => {
            this.getTableList()
          })
      })
