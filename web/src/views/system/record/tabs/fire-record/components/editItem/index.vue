...
export default {
  ...
  computed: {
    dialogHeader () {
      return Object.keys(this.itemInfo).length > 0 ? '修改档案' : '添加档案'
    }
  },
  methods: {
    closeDialog () {
    ...
    uploadFile (params, targetKey) {
      ...
      data.append('type', 'fm_file_manage')
    ...
    submitForm () {
      this.$refs.form.submitForm().then(() => {
        ...
        if (isEdit) {
          ...
          this.$store
            .dispatch('sinuo/fireControl/updateFireRecord', {
              ...formData,
              id
            })
            .then(() => {
          ...
          this.$store
            .dispatch('sinuo/fireControl/createFireRecord', formData)
            .then(() => {
              this.closeDialog()
              this.emitUpdateList()
            })
