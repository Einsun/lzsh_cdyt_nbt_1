          ...
          >
            <template slot-scope="scope">
              ...
              <template v-else-if="column.label==='附件详情'">
                <span class="watch-accessory">
                  <a :href="scope.row.file?scope.row.file.url:''" target="_blank" class="color-secondary cursor-pointer text-underline">查看附件</a>
                </span>
              </template>
...
export default {
  ...
  methods: {
    ...
    getTableList () {
      ...
      this.$store.dispatch('sinuo/fireControl/getRulesList', params).then(res => {
    ...
    deleteInfo (info) {
      this.$confirm('此操作将对其进行永远删除，是否继续？', '提示', {
        ...
        this.$store
          .dispatch('sinuo/fireControl/deleteRules', params)
          .then(res => {
            this.getTableList()
          })
      })
